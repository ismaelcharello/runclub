...
function renderRoutines() {
  ...
  routineList.innerHTML = routines.length ? routines.map((routine) => `<div class="exercise-card"><h3>${escapeHtml(routine.name)}</h3><div class="tags"><span class="tag">${escapeHtml(routine.type)}</span><span class="tag">Rascunho local</span></div></div>`).join('') : 'Nenhuma rotina salva neste dispositivo.';
}
document.querySelector('#save-routine').addEventListener('click', () => {
  ...
  const routines = JSON.parse(localStorage.getItem('pace-orbit-routines') || '[]');
  routines.unshift({ name, type }); localStorage.setItem('pace-orbit-routines', JSON.stringify(routines)); document.querySelector('#routine-name').value = ''; renderRoutines();
});
renderRoutines();
